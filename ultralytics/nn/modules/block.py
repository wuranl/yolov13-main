...
class IA_Net(nn.Module):
    """Input adaptive enhancement block for low-visibility scenes."""

    def __init__(self, c1, hidden=16):
        super().__init__()
        hidden = max(8, int(hidden))
        self.illumination = nn.Sequential(
            Conv(c1, hidden, 3, 1),
            Conv(hidden, c1, 3, 1, act=False),
        )
        self.detail = nn.Sequential(
            Conv(c1, hidden, 3, 1),
            Conv(hidden, c1, 3, 1, act=False),
        )
        self.gate = nn.Sequential(nn.Conv2d(c1, c1, 1, 1, 0), nn.Sigmoid())

    def forward(self, x):
        illum = torch.sigmoid(self.illumination(x))
        detail = torch.tanh(self.detail(x))
        enhanced = x * (1.0 + illum) + detail
        gate = self.gate(x)
        return gate * enhanced + (1.0 - gate) * x


class MSDA(nn.Module):
    """Multi-Scale Deformable Attention fusion block for neck feature aggregation."""

    def __init__(self, c_low, c_mid, c_high, c2, n=1, num_points=8, e=0.5):
        super().__init__()
        self.num_points = max(1, int(num_points))
        hidden = max(16, int(c2 * e))
        self.offset_range = 0.25

        self.downsample = nn.AvgPool2d(kernel_size=2)
        self.upsample = nn.Upsample(scale_factor=2, mode="nearest")
        self.low_proj = Conv(c_low, c_mid, 1, 1)
        self.mid_proj = Conv(c_mid, c_mid, 1, 1)
        self.high_proj = Conv(c_high, c_mid, 1, 1)
        self.fuse = Conv(3 * c_mid, c_mid, 1, 1)
        self.in_proj = Conv(c_mid, hidden, 1, 1)
        self.offset = nn.Conv2d(hidden, 2 * self.num_points, kernel_size=3, stride=1, padding=1)
        self.attn = nn.Conv2d(hidden, self.num_points, kernel_size=1, stride=1, padding=0)
        self.out_proj = Conv(hidden, c2, 1, 1)
    ...
    def forward(self, x):
        x_low = self.low_proj(self.downsample(x[0]))
        x_mid = self.mid_proj(x[1])
        x_high = self.high_proj(self.upsample(x[2]))
        x = self.fuse(torch.cat((x_low, x_mid, x_high), dim=1))
        feat = self.in_proj(x)
        b, _, h, w = feat.shape

        offsets = torch.tanh(self.offset(feat)) * self.offset_range
        weights = torch.softmax(self.attn(feat), dim=1)
        base_grid = self._build_base_grid(h, w, feat.device, feat.dtype)

        out = torch.zeros_like(feat)
        for i in range(self.num_points):
            offset = offsets[:, 2 * i : 2 * i + 2].permute(0, 2, 3, 1)
            grid = (base_grid + offset).clamp(-1.0, 1.0)
