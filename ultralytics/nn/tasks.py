...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f]
            hidden = int(args[0]) if args else max(16, c2 * 2)
            args = [c2, hidden]
        elif m is MSDA:
            legacy = False
            idxs = list(f) if isinstance(f, (tuple, list)) else [f, f, f]
            if len(idxs) < 3:
                idxs = (idxs + [idxs[-1]] * 3)[:3]
            c_low, c_mid, c_high = (ch[idxs[0]], ch[idxs[1]], ch[idxs[2]])
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            num_points = int(args[1]) if len(args) > 1 else 8
            e = float(args[2]) if len(args) > 2 else 0.5
            args = [c_low, c_mid, c_high, c2, n, num_points, e]
            n = 1
        elif m is HyperACE:
            legacy = False
            c1 = ch[f[1]]
