...
HELP_URL = "See https://docs.ultralytics.com/datasets for data formatting guidance."
...
def img2label_paths(img_paths):
    ...
    # Support both common Ultralytics layouts:
    # - .../images/*.jpg  -> .../labels/*.txt
    # - .../JPEGImages/*.jpg -> .../labels/*.txt
    sb = f"{os.sep}labels{os.sep}"  # /labels/ substring
    sa_candidates = (
        f"{os.sep}images{os.sep}",
        f"{os.sep}JPEGImages{os.sep}",
    )
    ...
    for x in img_paths:
        ...
        for sa in sa_candidates:
            if sa in x:
                mapped = sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt"
                break
        if mapped is None:
            # Fallback: place labels next to images if no known folder token exists.
            mapped = x.rsplit(".", 1)[0] + ".txt"
        label_paths.append(mapped)
    return label_paths


def check_file_speeds(files, threshold_ms=10, threshold_mb=50, max_files=5, prefix=""):
    """
    Check data file access speed and provide performance feedback.

    This function tests the access speed of data files by measuring ping (stat call) time and read speed.
    ...
    Examples:
        >>> from pathlib import Path
        >>> image_files = list(Path("data/JPEGImages").glob("*.jpg"))
...
def exif_size(img: Image.Image):
    ...
    if img.format == "JPEGImages":  # only support JPEGImages JPEGImages
        try:
            if exif := img.getexif():
    ...
    try:
        ...
        if im.format.lower() in {"jpg", "jpeg"}:
            with open(im_file, "rb") as f:
                f.seek(-2, 2)
                if f.read() != b"\xff\xd9":  # corrupt JPEGImages
                    ImageOps.exif_transpose(Image.open(im_file)).save(im_file, "JPEGImages", subsampling=0, quality=100)
                    msg = f"{prefix}{im_file}: corrupt JPEGImages restored and saved"
        nf = 1
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                assert max_cls < num_cls, (
                    f"Label class {int(max_cls)} exceeds data class count {num_cls}. "
...
def find_dataset_yaml(path: Path) -> Path:
    """
    Find and return the YAML file associated with a Detect, Segment or Pose data.
...
def check_det_dataset(dataset, autodownload=True):
    """
    Download, verify, and/or unzip a data if not found locally.

    This function checks the availability of a specified data, and if not found, it has the option to download and
    unzip the data. It then reads and parses the accompanying YAML data, ensuring key requirements are met and also
    resolves paths related to the data.

    Args:
        dataset (str): Path to the data or data descriptor (like a YAML file).
        autodownload (bool, optional): Whether to automatically download the data if not found.

    Returns:
        (dict): Parsed data information and paths.
    """
    ...
    # Resolve paths
    path = Path(extract_dir or data.get("path") or Path(data.get("yaml_file", "")).parent)  # data root
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            name = clean_url(dataset)  # data name with URL auth stripped
            LOGGER.info("")
            m = f"Dataset '{name}' JPEGImages not found, missing path '{[x for x in val if not x.exists()][0]}'"
            if s and autodownload:
                LOGGER.warning(m)
            else:
                m += f"\nNote data download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_FILE}'"
...
def check_cls_dataset(dataset, split=""):
    """
    Checks a classification data such as Imagenet.

    This function accepts a `data` name and attempts to retrieve the corresponding data information.
    If the data is not found locally, it attempts to download the data from the internet and save it locally.

    Args:
        dataset (str | Path): The name of the data.
        split (str, optional): The split of the data. Either 'val', 'test', or ''.

    Returns:
        (dict): A dictionary containing the following keys:

            - 'train' (Path): The directory path containing the training set of the data.
            - 'val' (Path): The directory path containing the validation set of the data.
            - 'test' (Path): The directory path containing the test set of the data.
            - 'nc' (int): The number of classes in the data.
            - 'names' (dict): A dictionary of class names in the data.
    """
    # Download (optional if data=https://file.zip is passed directly)
    ...
    if not train_set.is_dir():
        ...
        if image_files:
            ...
            LOGGER.info(f"Found {len(image_files)} JPEGImages in subdirectories. Attempting to split...")
            data_dir = split_classify_dataset(data_dir, train_ratio=0.8)
            train_set = data_dir / "train"
        else:
            LOGGER.error(f"No JPEGImages found in {data_dir} or its subdirectories.")
    val_set = (
        data_dir / "val"
    ...
    for k, v in {"train": train_set, "val": val_set, "test": test_set}.items():
        ...
        if v is None:
            ...
            if nf == 0:
                if k == "train":
                    raise FileNotFoundError(f"{dataset} '{k}:' no training JPEGImages found")
                else:
                    LOGGER.warning(f"{prefix} found {nf} JPEGImages in {nd} classes (no JPEGImages found)")
            elif nd != nc:
                LOGGER.error(f"{prefix} found {nf} JPEGImages in {nd} classes (requires {nc} classes, not {nd})")
            else:
                LOGGER.info(f"{prefix} found {nf} JPEGImages in {nd} classes ✅ ")
...
class HUBDatasetStats:
    """
    A class for generating HUB data JSON and `-hub` data directory.

    Args:
        path (str): Path to data.yaml or data.zip (with data.yaml inside data.zip). Default is 'coco8.yaml'.
        task (str): Dataset task. Options are 'detect', 'segment', 'pose', 'classify'. Default is 'detect'.
        autodownload (bool): Attempt to download data if not found locally. Default is False.
    ...
    Examples:
        ...
        >>> stats = HUBDatasetStats("path/to/coco8.zip", task="detect")  # detect data
        >>> stats = HUBDatasetStats("path/to/coco8-seg.zip", task="segment")  # segment data
        >>> stats = HUBDatasetStats("path/to/coco8-pose.zip", task="pose")  # pose data
        >>> stats = HUBDatasetStats("path/to/dota8.zip", task="obb")  # OBB data
        >>> stats = HUBDatasetStats("path/to/imagenet10.zip", task="classify")  # classification data
    ...
    def __init__(self, path="coco8.yaml", task="detect", autodownload=False):
        ...
        LOGGER.info(f"Starting HUB data checks for {path}....")
        ...
        if self.task == "classify":
            ...
            try:
                ...
                data["path"] = ""  # strip path since YAML should be in data root for all HUB datasets
    ...
    def _hub_ops(self, f):
        ...
        compress_one_image(f, self.im_dir / Path(f).name)  # save to data-hub

    def get_json(self, save=False, verbose=False):
        """Return data JSON for Ultralytics HUB."""
        ...
        def _round(labels):
            ...
            if self.task == "detect":
                ...
                raise ValueError(f"Undefined data task={self.task}.")
        ...
        if save:
            self.hub_dir.mkdir(parents=True, exist_ok=True)  # makes data-hub/
    ...
    def process_images(self):
        ...
        self.im_dir.mkdir(parents=True, exist_ok=True)  # makes data-hub/JPEGImages/
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = YOLODataset(img_path=self.data[split], data=self.data)
            with ThreadPool(NUM_THREADS) as pool:
                for _ in TQDM(pool.imap(self._hub_ops, dataset.im_files), total=len(dataset), desc=f"{split} JPEGImages"):
                    pass
        LOGGER.info(f"Done. All JPEGImages saved to {self.im_dir}")
        return self.im_dir
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    Examples:
        ...
        >>> for f in Path("path/to/data").rglob("*.jpg"):
    ...
    try:  # use PIL
        ...
        im.save(f_new or f, "JPEGImages", quality=quality, optimize=True)  # save
...
def save_dataset_cache_file(prefix, path, x, version):
    """Save an Ultralytics data *.cache dictionary x to path."""
