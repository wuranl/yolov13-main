...
# Ultralytics data *.cache version, >= 1.0.0 for YOLOv8
DATASET_CACHE_VERSION = "1.0.3"

IMG_FORMATS = 'bmp', 'dng', 'jpeg', 'jpg', 'mpo', 'png', 'tif', 'tiff', 'webp'  # include image suffixes,图片格式后缀
...
class YOLODataset(BaseDataset):
    ...
    Methods:
        cache_labels: Cache data labels, check JPEGImages and read shapes.
    ...
    Examples:
        >>> data = YOLODataset(img_path="path/to/JPEGImages", data={"names": {0: "person"}}, task="detect")
        >>> data.get_labels()
    """
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        """
        Cache data labels, check JPEGImages and read shapes.
    ...
    def get_labels(self):
        ...
        labels = cache["labels"]
        if not labels:
            LOGGER.warning(f"No JPEGImages found in {cache_path}, training may not work correctly. {HELP_URL}")
        self.im_files = [lb["im_file"] for lb in labels]  # update im_files

        # Check if the data is all boxes or all segments
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                ...
                "To avoid this please supply either a detect or segment data, not a detect-segment mixed data."
            )
            for lb in labels:
                lb["segments"] = []
        if len_cls == 0:
...
class YOLOMultiModalDataset(YOLODataset):
    ...
    Examples:
        >>> data = YOLOMultiModalDataset(img_path="path/to/JPEGImages", data={"names": {0: "person"}}, task="detect")
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            # the strategy of selecting negative is restricted in one data,
    ...
    def category_names(self):
        """
        Return category names for the data.
    ...
    def category_freq(self):
        """Return frequency of each category in the data."""
...
class GroundingDataset(YOLODataset):
    ...
    This data is designed for grounding tasks where annotations are provided in a JSON file rather than
    ...
    Examples:
        >>> data = GroundingDataset(img_path="path/to/JPEGImages", json_file="annotations.json", task="detect")
        >>> len(data)  # Number of valid JPEGImages with annotations
    """
    ...
    def get_img_files(self, img_path):
        ...
        Args:
            img_path (str): Path to the directory containing JPEGImages.
    ...
    def verify_labels(self, labels):
        """Verify the number of instances in the data matches expected counts."""
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        images = {f"{x['id']:d}": x for x in annotations["JPEGImages"]}
    ...
    @property
    def category_names(self):
        """Return unique category names from the data."""
...
class ClassificationDataset:
    ...
    Attributes:
        ...
        torch_transforms (callable): PyTorch transforms to be applied to the JPEGImages.
    ...
    Methods:
        ...
        __len__: Returns the total number of samples in the data.
        verify_images: Verifies all JPEGImages in data.
    """
    ...
    def __init__(self, root, args, augment=False, prefix=""):
        ...
        Args:
            root (str): Path to the data directory where JPEGImages are stored in a class-specific folder structure.
            args (Namespace): Configuration containing data-related settings such as image size, augmentation
                parameters, and cache settings.
            augment (bool, optional): Whether to apply augmentations to the data.
            prefix (str, optional): Prefix for logging and cache filenames, aiding in data identification.
        """
        ...
        self.cache_ram = args.cache is True or str(args.cache).lower() == "ram"  # cache JPEGImages into RAM
        if self.cache_ram:
            LOGGER.warning(
        ...
        self.cache_disk = str(args.cache).lower() == "disk"  # cache JPEGImages on hard drive as uncompressed *.npy files
        self.samples = self.verify_images()  # filter out bad JPEGImages
    ...
    def verify_images(self):
        ...
        try:
            ...
            if LOCAL_RANK in {-1, 0}:
                d = f"{desc} {nf} JPEGImages, {nc} corrupt"
            ...
            with ThreadPool(NUM_THREADS) as pool:
                ...
                for sample, nf_f, nc_f, msg in pbar:
                    ...
                    nf += nf_f
                    nc += nc_f
                    pbar.desc = f"{desc} {nf} JPEGImages, {nc} corrupt"
                pbar.close()
            if msgs:
                LOGGER.info("\n".join(msgs))
...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True): # path:'data\\JPEGImages\\bus.jpg', img_size:[640, 640]
        p = str(Path(path).resolve())  # os-agnostic absolute path, C:\\Users\\zkh\\Desktop\\yolov5\\data\\JPEGImages\\bus.jpg，获取到绝对路径
        if '*' in p:
            files = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        elif os.path.isfile(p):
            files = [p]  # files, ["C:\\Users\\zkh\\Desktop\\yolov5\\data\\JPEGImages\\bus.jpg"]  变成列表形式的文件
        else:
            raise Exception(f'ERROR: {p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]  #读取到files,以点为分割，获取到jpg是否包含在其中，判断是否是图片
        videos = [x for x in files if x.split('.')[-1].lower() in VID_FORMATS]#判断是否有视频格式的路径
        ni, nv = len(images), len(videos)  #获取到这个列表的长度

        self.img_size = img_size
        self.stride = stride
        self.files = images + videos
        self.nf = ni + nv  # number of files
        self.video_flag = [False] * ni + [True] * nv # [False] #视频标志，没有视频的话为false
        self.mode = 'image'
        self.auto = auto
        if any(videos):  #判断video有无值
    ...
    def __next__(self):
        if self.count == self.nf:   #count表示打印信息中的预测到第几张图片
            raise StopIteration
        path = self.files[self.count]

        if self.video_flag[self.count]:  #如果是视频，就执行下面操作
            # Read video
            self.mode = 'video'
            ...
            s = f'image {self.count}/{self.nf} {path}: '  #以便打印一些字符串信息，第几张图片和图片的路径

        # Padded resize
        #因为要求输入的图片是大小格式要求，所以就缩放图片，剩下的用黑色填补为self.img_size*self.img_size大小，但是auto是真是也可不填充
        img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0] # (640, 480, 3)

        # Convert
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB 转换图片格式
...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        return img_path, img, img0, self.cap, s
